...
import { ResearchJobData } from "@/services/research/founder-research-service";
...
export const ExecutiveDashboardView: React.FC = () => {
  ...
  const [researchJobs, setResearchJobs] = useState<ResearchJobData[]>([]);
  ...
  const loadExecutiveData = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to load live workspace data from Supabase.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const weeklyProgressPercent = tasks.length > 0 ? Math.round((completedTaskCount / tasks.length) * 100) : 65;
    ...
    <div className="space-y-8 select-none max-w-7xl mx-auto pb-16">
      ...
      {isLoading && (
        <Card className="p-12 border-[#E5E0D6] bg-card text-center space-y-4 shadow-sm">
          ...
          <h3 className="font-display font-bold text-lg text-foreground">Loading Executive Dashboard...</h3>
      ...
      {error && !isLoading && (
        <Card className="p-8 border-destructive/20 bg-destructive/5 text-center space-y-4 shadow-sm">
          ...
          <h3 className="font-display font-bold text-lg text-foreground">Supabase Database Error</h3>
          <p className="text-xs text-destructive max-w-md mx-auto">{error}</p>
          <Button onClick={loadExecutiveData} variant="outline" className="h-10 px-6 rounded-xl font-mono text-xs">
            Retry Sync
          </Button>
        </Card>
      )}
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="p-6 border-[#E5E0D6] bg-card space-y-4 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex items-baseline space-x-3">
                <span className="font-display font-bold text-5xl text-foreground">{growthScore}</span>
          ...
          <Card className="p-6 md:p-8 border-[#E5E0D6] bg-card space-y-4 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {tasks.slice(0, 4).map((task) => (
                <div key={task.id} className="p-4 rounded-xl border border-[#E5E0D6] bg-background space-y-2">
          ...
          <Card className="p-6 md:p-8 border-[#E5E0D6] bg-card space-y-4 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between border-b border-[#E5E0D6] pb-3">
              ...
              <Button onClick={() => navigate({ to: "/distribution" as any })} variant="ghost" size="sm" className="font-mono text-xs gap-1">
            ...
            {campaigns.length === 0 ? (
              <div className="py-8 text-center space-y-3">
                ...
                <p className="text-xs text-muted-foreground">No active campaigns in Supabase. Generate your first campaign via Growth Copilot.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {campaigns.map((cmp) => (
                  <div key={cmp.id} className="p-4 rounded-xl border border-[#E5E0D6] bg-background space-y-2">
                    <div className="flex items-center justify-between">
                      <Badge variant="outline" className="font-mono text-[9px] bg-primary/10 text-primary">
                        {cmp.type}
                      </Badge>
                      <span className="font-mono text-[10px] text-emerald-600 font-bold uppercase">{cmp.status}</span>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="p-6 border-[#E5E0D6] bg-card space-y-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between border-b border-[#E5E0D6] pb-3">
                ...
                <Link to="/research" className="font-mono text-xs text-primary font-bold flex items-center gap-1 hover:underline">
              ...
              <div className="space-y-3">
                <div className="p-3 rounded-xl border border-[#E5E0D6] bg-background flex items-center justify-between text-xs">
                  <div>
                    ...
                    <span className="text-[10px] text-muted-foreground font-mono">Completed • 96% ICP Match</span>
                  </div>
                  <Badge variant="outline" className="font-mono text-[9px] bg-emerald-500/10 text-emerald-600 border-emerald-500/20">Verified</Badge>
                </div>
                <div className="p-3 rounded-xl border border-[#E5E0D6] bg-background flex items-center justify-between text-xs">
                  <div>
                    <p className="font-bold text-foreground">Revix Growth Persona Map</p>
                    <span className="text-[10px] text-muted-foreground font-mono">Completed • 92% ICP Match</span>
                  </div>
                  <Badge variant="outline" className="font-mono text-[9px] bg-emerald-500/10 text-emerald-600 border-emerald-500/20">Verified</Badge>
                </div>
              </div>
              ...
              <div className="space-y-3">
                {recommendations?.highestRisk ? (
                  <div className="p-4 rounded-xl border border-amber-500/30 bg-amber-500/5 space-y-1 text-xs">
                    <span className="font-mono text-[10px] text-amber-700 uppercase font-bold">HIGHEST RISK VECTOR</span>
                    <h4 className="font-bold text-foreground">{recommendations.highestRisk.title}</h4>
                    <p className="text-muted-foreground">{recommendations.highestRisk.riskFactor}</p>
                    <p className="text-emerald-700 font-semibold pt-1">Mitigation: {recommendations.highestRisk.mitigationStrategy}</p>
                  </div>
                ) : (
            ...
            {recommendations?.quickWins ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {recommendations.quickWins.map((win, idx) => (
                  <div key={idx} className="p-4 rounded-xl border border-[#E5E0D6] bg-background space-y-1 text-xs">
                    <div className="flex items-center justify-between">
                      <h4 className="font-bold text-foreground">{win.title}</h4>
                      <span className="font-mono text-[10px] text-primary font-bold">{win.estimatedTimeHours}h Effort</span>
                    </div>
          ...
          <Card className="p-6 md:p-8 border-[#E5E0D6] bg-card space-y-4 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="space-y-3 font-mono text-xs">
              <div className="p-3 rounded-xl border border-[#E5E0D6] bg-background flex items-center justify-between">
                <span className="text-foreground">✓ Growth Strategy Agent synthesized 12-section playbook</span>
                <span className="text-muted-foreground text-[10px]">Just now</span>
              </div>
              <div className="p-3 rounded-xl border border-[#E5E0D6] bg-background flex items-center justify-between">
                <span className="text-foreground">✓ Execution Planner compiled DAG Operating Plan into Supabase</span>
                <span className="text-muted-foreground text-[10px]">10m ago</span>
              </div>
              <div className="p-3 rounded-xl border border-[#E5E0D6] bg-background flex items-center justify-between">
                <span className="text-foreground">✓ Synced active workspace metrics with live Supabase database</span>
